...
def get_all_images(dir_name, sort_by, sort_order, keyword, tab_base_tag_box, img_path_depth, ranking_filter, ranking_filter_min, ranking_filter_max, aes_filter_min, aes_filter_max, score_type, exif_keyword, negative_prompt_search, use_regex, case_sensitive,username):
    dir_name = dir_name+'/'+username
    global current_depth
    logger.debug(f"get_all_images:{dir_name}")
    current_depth = 0
...
def get_image_thumbnail(image_list,username):
    logger.debug("get_image_thumbnail:",username)
    optimized_cache = os.path.join(tempfile.gettempdir(),username,"optimized")
...
def get_image_page(img_path, page_index, filenames, keyword, sort_by, sort_order, tab_base_tag_box, img_path_depth, ranking_filter, ranking_filter_min, ranking_filter_max, aes_filter_min, aes_filter_max, score_type, exif_keyword, negative_prompt_search, use_regex, case_sensitive, image_reward_button,request:gr.Request):
    ...
    filenames = get_all_images(img_path, sort_by, sort_order, keyword, tab_base_tag_box, img_path_depth, ranking_filter, ranking_filter_min, ranking_filter_max, aes_filter_min, aes_filter_max, score_type, exif_keyword, negative_prompt_search, use_regex, case_sensitive,username)
    ...
    if opts.image_browser_use_thumbnail:
        thumbnail_list = get_image_thumbnail(image_list,username)
    else:
...
def on_ui_tabs():
    ...
    return (image_browser, "Image Browser", "images_history"),
